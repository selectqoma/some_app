...
def usage():
    ...
    print('\nResults will be stored in ~/.ros/rgb_dataset')
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(rospy.myargv()[1:], 'hf:', ['help', 'file='])
        filepath = None
        for opt, arg in opts:
            if opt in ('-h', '--help'):
                usage()
    ...
    out_path = os.path.join(os.path.expanduser('~'), '.ros', 'frames_to_annotate')
    ...
    for topic, msg, t in bag.read_messages(topics=topic_list):
        if 'ptz/state' in topic:
            ...
            datetime = time.strftime('%Y%m%dT%H%M%S', time.localtime(msg.header.stamp.to_sec()))
            fn = '{}_rgb_image_pose={}_{}_{}.png'.format(datetime, pose['pan'], pose['tilt'], pose['zoom'])
            cv2.imwrite(os.path.join(out_path, 'rgb', fn), img)
        elif 'ir/normalized/image_raw' in topic:
            img = CvBridge().imgmsg_to_cv2(msg)
            datetime = time.strftime('%Y%m%dT%H%M%S', time.localtime(msg.header.stamp.to_sec()))
            fn = '{}_ir_image_pose={}_{}_{}.png'.format(datetime, pose['pan'], pose['tilt'], pose['zoom'])
